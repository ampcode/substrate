...
| `--pki-delivery projected\|agent` | `--pki-delivery projected\|agent`, or `ATE_PKI_DELIVERY` | How pods receive certificates and trust bundles (default `projected`). `agent` uses podcert-agent sidecars and ConfigMaps for clusters without `certificates.k8s.io/v1beta1`, such as AKS and EKS; it excludes `--setup-csi` and the sdsmint egress variants |
